...
def read(iso_data, datafiles, postfix="old"):
    ...
    if postfix == "old":
        ...
        dataset['public_participation'] = \
            _read_participation(pp_workbook, iso_data,
                                datafiles['pp_xlsx_sheet'])
    return dataset
...
def _read_answers(a_workbook, iso_data, sheet_name, years):
    ...
    headers = {x: _lookup(sheet, x, 1) for x in range(1, width + 1)}
    rows = []
    for y in range(2, height + 1):
        row = {headers[x]: _lookup(sheet, x, y) for x in range(1, width + 1)}
        rows.append(row)
    # Verify the data
    ...
    for row in rows:
        ...
        assert type(name) is unicode, \
            '[%s/%s] Invalid country name %s' % (name, year, unicode(name))
        name = name.strip()
        ...
        assert type(year) is int, \
            '[%s/%s] Invalid year %s (%s)' % (name, year, unicode(year), type(year))
        assert year in years, \
            '[%s/%s] Unexpected value of "year": %s' % (name, year, year)
        ...
        for key, value in row.items():
            ...
            if question_label.match(key) is not None:
                error_string = \
                    '[%s/%s] Invalid value for %s: %s' % (name, year,
                                                          key, value)
                # Verify the value
                if key[-1] == 'l':
                    assert value in [None, 'a', 'b', 'c', 'd', 'e'], \
                        error_string
                else:
                    if value is None:
                        # Blank becomes -1
...
def _read_regions(g_workbook, iso_data, sheet_name):
    ...
    while True:
        ...
        while True:
            ...
            if not country_name:
                break
            assert country_name in iso_data, \
                '[Groupings/%s] I have no ISO-3116 mapping for country name "%s". Please add one to the ISO mappings file.' % (sheet_name, country_name)
...
def _read_availability(av_workbook, iso_data, sheets):
    ...
    for sheet_name in sheets:
        ...
        for x in range(1, len(column_headers) + 1):
            ...
            assert column_header == column_headers[x-1][0], \
                'Bad column header on sheet %s: Got "%s" but expected "%s".' % \
                (sheet_name, column_header, column_headers[x-1])
        ...
        for y in range(2, height + 1):
            ...
            for x in range(2, len(column_headers) + 1):  # <-- Mind these off-by-ones! x is column number not list index.
                ...
                assert value in valid, \
                    'Invalid value in %s (%s, %s): %s' % \
                    (sheet_name, name, column_headers[x-1], value)
...
def _read_participation(pp_workbook, iso_data, sheet_name):
    ...
    vtypes = {
        'Score': 'score',
        'Letter': 'letter',
        'Comments and Citations in support of the response': 'comments'
    }
    height = sheet.max_row
    width = sheet.max_column
    ...
    for y in range(3, height + 1):
        ...
        for x in range(2, width + 1):
            ...
            if not value:
                value = ''
            value = value.strip()
            question = str(headers[x][0]).strip()
            vtype = str(headers[x][1]).strip()
            vkey = vtypes[vtype]
            if vkey == 'letter':
                value = value.replace('.', '')
            if question in data:
...
def _parse_int_list(int_list):
    ...
    if type(int_list) is float \
       or type(int_list) is int \
       or type(int_list) is long:
